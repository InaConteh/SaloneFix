...
- **Authorization is server-side only.** Use `require_roles([...])` from `app/core/dependencies.py` on every non-public endpoint, and re-check ownership/assignment inside the service (e.g. an officer can only act on incidents assigned to their institution). Frontend role gating is UX only, never a control.
